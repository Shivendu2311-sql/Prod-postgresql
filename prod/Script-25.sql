...
BEGIN
    ...
    latest_dispositions AS (
        ...
        WHERE  ad.txt_lead_disposition is not null --and
         --cast(ad.last_modified_date as date) BETWEEN start_date AND end_date
    ...
    DispositionCounts AS (
        SELECT
           ...
           COUNT(*) FILTER (WHERE ad.txt_lead_disposition IN ('Internet Only Sale', 'Wireless Only Sale', 'Wireless and Internet Sale')) AS sales_conversion_count,
        COUNT(*) FILTER (WHERE ad.txt_lead_disposition IN ('Decision Maker Not Home', 'No Answer')) AS comeback_count,
        COUNT(*) FILTER (WHERE ad.txt_lead_disposition = 'Follow up Appointment') AS to_be_visited_count,
        COUNT(*) FILTER (WHERE ad.txt_lead_disposition IN (
            ...
            )) AS do_not_knock_count
        FROM
            pgadmin."Prospect_partition" p
        JOIN "mapping"."Territory" t ON p.zip_cd = t.zip_cd and t.expiration_date >= current_date and t.isactive is not false  

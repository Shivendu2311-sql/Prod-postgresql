...
BEGIN
 ...
 territory_coords AS (
    ...
    territory_geometries AS (
		...
		AND ad.txt_agent_id = get_lead_counts_nds_2.agent_id
    ...
    matching_audience_ids AS (
        ...
        JOIN
            mapping."Territory" t ON p.zip_cd = t.zip_cd AND t.expiration_date >= CURRENT_DATE AND t.isactive IS NOT FALSE
        JOIN
        ...
        WHERE 
          ld.txt_lead_disposition is not null and tm.agent_id = get_lead_counts_nds_2.agent_id  and 
          ST_Contains(
                tg.geom,
    ...
    SELECT
        ...
        COUNT(CASE WHEN txt_lead_disposition IN ('Internet Only Sale', 'Wireless Only Sale', 'Wireless and Internet Sale') THEN 1 END) 
            FILTER (WHERE last_modified_date IS NULL OR (last_modified_date BETWEEN start_date AND end_date)),

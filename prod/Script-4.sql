-- get_icon_color_json
-- DROP FUNCTION public.get_icon_color_json(json, varchar);

CREATE OR REPLACE FUNCTION public.get_icon_color_json(p_input_json json, p_state_cd character varying)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        -- Construct the dynamic query to get the icon(s) based on audience_id and p_state_cd
        query_builder := format('
            ...
            INNER JOIN pgadmin."map_link_table" m ON
                (CASE
                    WHEN ((p.unit_designator_cd IS NOT NULL AND p.unit_designator_cd != '''') OR
                         ...
                         CASE
                             WHEN p.isfiber = true THEN ''mdu_fiber''
                             WHEN p.pstpd_wrls_subsrptn_sts_cd = ''A'' THEN ''mdu_wireless''
                             ELSE ''mdu''
                         END
                    ELSE
                         CASE
                             WHEN p.isfiber = true THEN ''sfu_fiber''
            ...
            WHERE p.audience_id = %L
              AND p.state_cd = %L
            LIMIT 1', v_audience_id, p_state_cd);

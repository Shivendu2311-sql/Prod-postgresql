WITH DecodedFilters AS (
    ...
    DealerInfo AS(
        SELECT dealer_id
        FROM "mapping"."Teams"
        WHERE agent_id = '005Pr000007rXHxIAM'
    ),
    AgentInfo AS (
        SELECT agent_id
        FROM "mapping"."Teams"
    ...
    CASE 
        WHEN (
            ((unit_designator_cd IS NOT NULL AND unit_designator_cd != '') OR 
             (unit_nbr IS NOT NULL AND unit_nbr != ''))
        ...
        ELSE
            CASE
                ...
                ELSE 'other'
            END
    END AS lead_type, p.state_cd,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Internet Only Sale', 'Wireless Only Sale', 'Wireless and Internet Sale') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN '2025-01-01' AND '2025-01-20')) AS sales_conversion_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Decision Maker Not Home', 'No Answer') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN '2025-01-01' AND '2025-01-20')) AS comeback_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Follow up Appointment') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN '2025-01-01' AND '2025-01-20')) AS to_be_visited_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN (
        ...
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN '2025-01-01' AND '2025-01-22')) AS do_not_knock_count
FROM
     ...
     group by unit_designator_cd, unit_nbr, isfiber,pstpd_wrls_subsrptn_sts_cd, p.state_cd
)
        SELECT *
FROM DispositionCounts
WHERE 
    ('all' = 'all') --OR lead_type IN ('fiber', 'wireless', 'other'));
...
CREATE OR REPLACE FUNCTION public.get_agent_and_team_counts_3(p_agent_id text, start_date date, end_date date, p_state_cd text[] DEFAULT NULL::text[], p_lead_type text DEFAULT 'all'::text, p_territory_ids character varying[] DEFAULT NULL::character varying[])
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    COUNT(DISTINCT (p.audience_id, tm.agent_id)) AS my_lead_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Internet Only Sale', 'Wireless Only Sale', 'Wireless and Internet Sale') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN start_date AND end_date)) AS sales_conversion_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Decision Maker Not Home', 'No Answer') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN start_date AND end_date)) AS comeback_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN ('Follow up Appointment') THEN 1 END)
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN start_date AND end_date)) AS to_be_visited_count,
    COUNT(CASE WHEN ad.txt_lead_disposition IN (
        'Decision Maker Not Home - Final', 'No Answer - Final', 'Competitor Loyalty', 'Discount Requested', 'Employee/Retiree',
        'Existing Customer', 'Health Concern', 'Installation Timing', 'Language Barrier - Spanish', 'Language Barrier - Other',
        'Moving', 'Not Interested', 'Not Qualified', 'Plan Limitations', 'Price Sensitivity', 'Under Contract',
        ...
        FILTER (WHERE ad.last_modified_date IS NULL OR (ad.last_modified_date BETWEEN start_date AND end_date)) AS do_not_knock_count
FROM
            pgadmin."Prospect_partition" p
        LEFT JOIN latest_dispositions ad ON p.audience_id = ad.txt_audience_id AND ad.rn = 1
        ...
        --AND t.territory_id IN (
         --   SELECT territory_id
          --  FROM "mapping"."Territory"
           ...
           -- )
        --)
       -- AND(CAST(ad.last_modified_date AS date) BETWEEN start_date AND end_date OR ad.last_modified_date is null)

...
CREATE OR REPLACE FUNCTION public.get_master_dealer_dashboard(p_sfid text, p_start_date date, p_end_date date)
...
BEGIN
   ...
   DROP TABLE IF EXISTS temp_account_hierarchy;
    -- Step 1: Recursive CTE to build account hierarchy, optimized to reduce overhead, below
    ...
    WHERE 
        ...
        CAST(ad.last_modified_date AS date) BETWEEN p_start_date AND p_end_date --and dealer_type in ('Agent')

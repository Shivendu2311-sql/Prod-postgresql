WITH DecodedFilters AS (
    ...
    TerritoryInfo AS (
        ...
        WHERE t.agent_id IN (SELECT agent_id FROM AgentInfo)
          ...
          AND
                  t2.territory_id = '23aa5a52-74a0-4889-9565-86dac0b49e6e'
    ),
    territory_coords AS (
        SELECT
            t.territory_id,
    ...
    DispositionCounts AS (
        ...
        AND CAST(ad.last_modified_date AS date) BETWEEN '2025-01-01' AND '2025-01-16'
...
select * from DispositionCounts


CREATE OR REPLACE FUNCTION public.get_agent_and_team_counts_2(p_agent_id text, start_date date, end_date date, p_state_cd text[] DEFAULT NULL::text[], p_lead_type text DEFAULT 'all'::text, p_territory_ids character varying[] DEFAULT NULL::character varying[])
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    latest_dispositions AS (
        ...
        FROM pgadmin."AgentDisposition" ad
        WHERE ad.txt_lead_disposition is not null and CAST(ad.last_modified_date AS date) between start_date and end_date
    ),
    DispositionCounts AS (
        SELECT
            COUNT(DISTINCT tm.agent_id) AS total_agents,
...
SELECT 
    pid, query, wait_event , wait_event_type ,
    usename, 
    application_name, 
    client_addr, 
    state, 

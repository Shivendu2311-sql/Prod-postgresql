CREATE OR REPLACE FUNCTION public.get_lead_counts_nds_3(
    agent_id text, 
    start_date date, 
    end_date date, 
    p_state_cd text[] DEFAULT NULL::text[], 
    p_lead_type text DEFAULT 'all'::text, 
    p_territory_ids character varying[] DEFAULT NULL::character varying[]
)
RETURNS jsonb
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    territory_geometries AS (
     ...
     latest_dispositions AS (
		...
		AND ad.txt_agent_id = get_lead_counts_nds_3.agent_id
    ),
    matching_audience_ids AS (
        ...
        LEFT JOIN
            latest_dispositions ld ON p.audience_id = ld.txt_audience_id
        JOIN
            mapping."Teams" tm ON tm.territory_id = t.territory_id
        JOIN
            CombinedFilters cf ON (cf.combined_filter)::text LIKE '%' || quote_literal(t.zip_cd) || '%'
        WHERE 
            ld.txt_lead_disposition IS NOT NULL
            AND tm.agent_id = get_lead_counts_nds_3.agent_id
            AND (p_state_cd IS NULL OR p.state_cd = ANY(p_state_cd))
            AND (
        CASE 
            WHEN (
    ...
    SELECT
        ...
        AND (last_modified_date IS NULL OR cast(last_modified_date as date) BETWEEN start_date AND end_date) 
        THEN 1 END),
    COUNT(CASE 
    ...
    RETURN jsonb_build_object(
        'total_lead_count', COALESCE(total_lead_count, 0),
        'sales_conversion', COALESCE(sales_conversion_count, 0),
        'comeback', COALESCE(comeback_count, 0),
        'to_be_visited', COALESCE(to_be_visited_count, 0),
        'do_not_knock', COALESCE(do_not_knock_count, 0)
    );
END;
$function$
;

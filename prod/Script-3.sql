-- get_lead_counts_nds_2_test_shiva_on_Prod
-- DROP FUNCTION public.get_lead_counts_nds_2_test_shiva(text, date, date, _text, text, _varchar);

CREATE OR REPLACE FUNCTION public.get_lead_counts_nds_2_test_shiva(agent_id text, start_date date, end_date date, p_state_cd text[] DEFAULT NULL::text[], p_lead_type text DEFAULT 'all'::text, p_territory_ids character varying[] DEFAULT NULL::character varying[])
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
 ...
 territory_coords AS (
    ...
    territory_geometries AS (
		...
		---AND ad.txt_agent_id = get_lead_counts_nds_2_test_shiva.agent_id
        UNION ALL
        SELECT
            ad.txt_audience_id,
    ...
    matching_audience_ids AS (
        ...
        WHERE
            tm.agent_id = get_lead_counts_nds_2_test_shiva.agent_id          
    ...
    SELECT
        ...
        COUNT(CASE WHEN txt_lead_disposition IN ('Internet Only Sale', 'Wireless Only Sale', 'Wireless and Internet Sale') THEN 1 END),
        COUNT(CASE WHEN txt_lead_disposition IN ('Decision Maker Not Home', 'No Answer') THEN 1 END),
        COUNT(CASE WHEN txt_lead_disposition IN ('Follow up Appointment') THEN 1 END),
